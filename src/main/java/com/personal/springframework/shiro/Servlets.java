...
public class Servlets {
	...
	private final static String[] staticFiles = StringUtils.split(Constant.WEB_STATICFILE, ",");
	/**
	 * 设置客户端缓存过期时间 的Header.
	 */
		...
		if (staticFiles == null){
			try {
				throw new Exception("检测到“app.properties”中没有配置“web.staticFile”属性。配置示例：\n#静态文件后缀\n"
					+"web.staticFile=.eot,.svg,.ttf,.woff,.woff2,.css,.js,.png,.jpg,.gif,.jpeg,.bmp,.ico,.swf,.psd,.htc,.crx,.xpi,.exe,.ipa,.apk");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
